...
from vllm_ascend.spec_decode.adaptive_verification import AdaptiveVerificationManager
...
class AscendDSparkProposer(AscendDflashProposer):
    ...
    def __init__(
        ...
        # Cost-aware adaptive verification is optional. The manager owns the
        # startup cost curves, stale-confidence D2H buffers and per-step budget
        # policy; this proposer only computes the confidence-head output.
        ...
        if dynamic_spec_config.method == "dspark":
            self.adaptive_verification = AdaptiveVerificationManager(
                max_num_reqs=self.max_batch_size,
                max_num_batched_tokens=self.max_num_tokens,
                num_speculative_tokens=self.num_speculative_tokens,
                device=device,
                method_params=dynamic_spec_config.method_params,
                pin_memory=getattr(runner, "pin_memory", False),
            )
    ...
    def update_num_verify_tokens(
        ...
        """Predict live conditional confidence and allocate the budget."""
        if self.adaptive_verification is None:
            self._dspark_num_verify_tokens = None
            return

        confidence_logits = self._compute_confidence_logits(last_hidden_states, draft_token_ids, num_reqs)
        # Confidence-head output is conditional acceptance probability after sigmoid.
        # The manager derives survival probabilities with cumprod.
        confidence_probs = confidence_logits.sigmoid().clamp_(min=1e-6, max=1.0)
        self._dspark_num_verify_tokens = self.adaptive_verification.update(confidence_probs)
